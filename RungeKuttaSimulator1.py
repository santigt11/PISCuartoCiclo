...
class RungeKuttaSimulator1:
    def __init__(self):
        ...
        self.proporcion_hombres = 0.52  # proporción de hombres en la población estudiantil
    ...
    def aprobados_runge_kutta(self, total_estudiantes, t0, t_final, h, opcion):
        if opcion == "hombres":
            delta = 0.30
            estudiantes = int(total_estudiantes * self.proporcion_hombres)
        elif opcion == "mujeres":
            delta = 0.20
            estudiantes = int(total_estudiantes * (1 - self.proporcion_hombres))
        else:
            delta = 0.25
    ...
    def simular_ciclos(self, estudiantes_inicial, año_inicio, año_fin, opcion):
        ...
        for año in range(año_inicio, año_fin + 1):
            for ciclo in [1, 2]:
                if opcion == "hombres":
                    gamma1 = 0.07
                    factor = self.proporcion_hombres
                elif opcion == "mujeres":
                    gamma1 = 0.06
                    factor = 1 - self.proporcion_hombres
                else:
                    gamma1 = self.gamma
                    factor = 1

                nuevos_ingresos = int(estudiantes[-1] * gamma1 * factor)
    ...
    def obtener_resultados(self, estudiantes_inicial, año_inicio, año_fin, opcion):
        return self.simular_ciclos(estudiantes_inicial, año_inicio, año_fin, opcion)